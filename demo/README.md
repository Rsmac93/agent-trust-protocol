...
reputation), and finally the validator cranks emissions and claims its VOLT
work reward.
...
- The repo-root `.env` has `PRIVATE_KEY` for the deployer/owner account (the
  same one used to deploy — it holds the VOLT premine and owns the contracts).
- Node 18+.
...
(`voltpass-sdk` is pulled in via a `file:../sdk` dependency — if you've
...
1. **Validator setup** (as deployer/owner) — generates a fresh validator
   ...
   `2 * Staking.MIN_BOND` VOLT, approves `Staking`, and calls
...
2. **Agent registration** — generates a fresh agent wallet, funds it with
   ...
   (`VoltPass.registerAgent`), paying the ETH registration fee. No VOLT is
...
3. **Mock trading loop** — 10 ticks of a simple simulated random-walk price
   ...
   `VoltPass.logReceipt(...)`, which canonicalizes it to a `keccak256` hash
   and notarizes that hash on-chain (self-reported lane — free, no
   validators required).

4. **Validator attestation** — the validator wallet (via a second
   `VoltPass` instance, its own key) calls `attestReceipt` on a subset of
...
6. **Optional finale (reward claim)** — warps chain time forward 2 days
   ...
   `claimValidator(epoch)` — printing the VOLT amount claimed. This step is
...
receiptHash)` and `getAttestor(agentId, receiptHash)` on the `VoltPass`
...
`VoltPassConfig`, never hardcoded into the SDK.
...
- `src/abi-extra.ts` — minimal ABIs for `Staking`, `VoltToken` (ERC20),
