...
const rep = await new AgentTrust({ registryAddress: '0x...' }).getAgent(agentId);
```

## Design decisions (from DX review)
- **No token friction**: registration in native ETH; the AGT token exists only on the validator/staking side
